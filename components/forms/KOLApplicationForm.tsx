...
import { Check, AlertCircle, ArrowRight, Wallet, ChevronDown, Eye, EyeOff, Link2, Unlink } from 'lucide-react';
...
export default function KOLApplicationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only sign if wallet is connected
        ...
        <div className="relative text-center py-16 px-6">
          ...
          {submittedWallet && (
            <p className="text-xs text-muted-foreground font-mono bg-white/[0.03] border border-white/[0.06] rounded-lg px-4 py-2 inline-block">
              {submittedWallet.slice(0, 8)}...{submittedWallet.slice(-8)}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="relative p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3.5 text-sm font-semibold rounded-xl bg-primary text-black hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 hover:shadow-[0_0_25px_rgba(0,255,136,0.3)]"
          >
            {isSubmitting ? (
              'Submitting...'
            ) : (
