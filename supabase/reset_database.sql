-- XSCAN Database Reset Script
-- Run this to clean up and recreate all tables
...
DROP FUNCTION IF EXISTS recalculate_kol_stats(UUID) CASCADE;
DROP FUNCTION IF EXISTS update_kol_stats_trigger() CASCADE;
...
CREATE INDEX idx_kols_twitter_handle ON kols(twitter_handle);
CREATE INDEX idx_kols_winrate ON kols(winrate DESC);
CREATE INDEX idx_kols_avg_multiplier ON kols(avg_multiplier DESC);
CREATE INDEX idx_kols_is_active ON kols(is_active);
...
CREATE INDEX idx_calls_kol_id ON calls(kol_id);
CREATE INDEX idx_calls_token_address ON calls(token_address);
CREATE INDEX idx_calls_entry_timestamp ON calls(entry_timestamp DESC);
CREATE INDEX idx_calls_ath_multiplier ON calls(ath_multiplier DESC);
...
CREATE INDEX idx_applications_twitter ON applications(twitter_handle);
CREATE INDEX idx_applications_wallet ON applications(wallet_address);
...
CREATE INDEX idx_price_history_call_id ON price_history(call_id);
...
CREATE POLICY "Applications are viewable" ON applications
    FOR SELECT USING (true);
